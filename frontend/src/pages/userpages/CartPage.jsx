...
const CartPage = () => {
  ...
  return (
    <div className="min-h-screen min-w-screen pb-10 px-4 md:px-27 bg-gray-200 flex flex-col gap-5 pt-20">
      <h2 className="text-2xl font-bold">Shopping Cart</h2>

      {items.length === 0 ? (
        <div className="w-full h-[50vh] flex items-center justify-center font-bold text-2xl">
          Cart is Empty
        </div>
      ) : (
        <div className="flex flex-col md:flex-row md:gap-6 gap-10">
          <div className="md:w-3/5 flex flex-col gap-6">
            {items.map((item) => (
              <div key={item._id} className="bg-white flex p-3 gap-4 rounded-xl">
                <img
